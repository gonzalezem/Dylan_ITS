...
CodeFlower.prototype.update = function(json) {
  ...
  //  .style("stroke", function(d) { return d.target.color; })  //add this one to have colors on the path/link as well
    ...
    .style("stroke-width", 4);  // path/link size
  ...
  this.node.transition()
    .attr("r", function(d) { return d.children ? 8 : Math.pow(d.size, 1/3) +5  || 1; }); // the first number gives the diameters of the nodes when there is no size. The second gives give the size of the last children.
...
CodeFlower.prototype.mouseover = function(d) {
  this.text.attr('transform', 'translate(' + d.x + ',' + (d.y - 5 - (d.children ? 3.5 : Math.sqrt(d.size) / 2)) + ')')
    .text(d.name + ": " + d.size + " transcripts")
    .style('display', null);
};
